"""Snapshot exporter for the Quarto frontend."""
...
ROOT = Path(os.environ.get("BE_LEADERBOARD_DATA_ROOT", SITE.parent)).resolve()
LABELS = {
    "bacteria_2024_mz10": "Urinary Pathogens · LC-MS/MS",
    "colon_3041": "Colon 3041",
    "jdlber_sle_maldi": "SLE · MALDI",
...
s.correction_code,s.model_code,
...
datasets = [
    {
        ...
        "runs": sum(r["dataset"] == k for r in rows),
        "protocols": sorted({r["evaluation_protocol"] for r in rows if r["dataset"] == k and r["evaluation_protocol"]}),
    }
    for k in keys
]
payload = {
    "stats": {
        ...
        "runs": len(rows),
        "protocols": len({r["evaluation_protocol"] for r in rows if r["evaluation_protocol"]}),
        "contributors": len({r["username"] for r in rows if r["username"]}),
    },
    "datasets": datasets,
    "leaderboard": rows,
    "backend": {
        "url": os.environ.get("BE_GRADIO_URL", "http://127.0.0.1:7899").rstrip("/"),
        "submit_endpoint": "submit_real",
        "logs_endpoint": "read_latest_run_logs",
        "stop_endpoint": "stop_real",
    },
}
