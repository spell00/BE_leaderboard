...
class BatchCVHeadSweepTrainer:
    ...
    def objective(self, trial):
        ...
        for fold, (fold_train_idx, fold_valid_idx) in enumerate(self.splits, start=1):
            ...
            for head_type in available_heads:
                ...
                try:
                    ...
                    print(f"[fold {fold}] head={head_type} train_mcc={tr_mcc:.4f} valid_mcc={vl_mcc:.4f}", flush=True)
        ...
        ae_avg = float(np.mean(ae_mccs)) if ae_mccs else float("-inf")
        head_avg = valid_mcc if np.isfinite(valid_mcc) else float("-inf")

        # best_valid_mcc = MAX(head classifier CV valid MCC, AE-internal classifier
        # MCC); best_model = ARGMAX of the same pair. Both are validation-set
        # metrics (no test leakage) - we simply keep whichever model family is
        # actually stronger for this trial's config, instead of always trusting
        # the downstream head classifier.
        if ae_avg >= head_avg:
            best_valid_mcc = ae_avg
            best_model = "ae_classifier"
        else:
            best_valid_mcc = head_avg
            best_model = "head"

        trial.set_user_attr("head_valid_mcc", head_avg)
        trial.set_user_attr("ae_classifier_mcc", ae_avg)
        trial.set_user_attr("best_valid_mcc", best_valid_mcc)
        trial.set_user_attr("best_model", best_model)

        if best_valid_mcc > self.best_valid_mcc:
            self.best_valid_mcc = best_valid_mcc
            self.best_head_type = head_type if best_model == "head" else "ae_classifier"
            self.best_head_params = head_params_by_type.get(head_type, {}) if best_model == "head" else {}

        print(
            f"[trial objective] best_head={head_type} head_valid_mcc={head_avg:.4f} "
            f"ae_classifier_mcc={ae_avg:.4f} best_model={best_model} best_valid_mcc={best_valid_mcc:.4f}",
            flush=True
        )
