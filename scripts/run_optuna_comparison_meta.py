...
def _main(argv=None) -> int:
    ...
    metadata.update({
        ...
        "cross_test": 1,
        "fixed_test_labels": "monitoring_only_excluded_from_selection",
    })
    ...
    if args.resume:
        for dataset_id, study in studies.items():
            ...
            for trial in running:
                ...
                print(
                    f"[optuna-comparison] marked interrupted {dataset_id} "
                    f"trial {trial.number} FAIL before resume",
                    flush=True,
                )
    ...
    completed_validation_steps = set()
    validation_path = args.output_dir / "validation_solutions.jsonl"
    if validation_path.exists():
        completed_validation_steps = {
            int(json.loads(line)["solution_step"])
            for line in validation_path.read_text().splitlines() if line.strip()
        }
    if validation_ids and args.validation_eval_policy != "never":
        validation_datasets = {
            dataset_id: hp_search.load_dataset(dataset_id) for dataset_id in validation_ids
        }
        validation_fixed_tests = {
            dataset_id: hp_search.load_fixed_test_dataset(dataset_id)
    ...
    # A contract-preserving salvage may import completed Optuna trials and their
    # durable solution ledger into a new W&B run. Replay those monitoring rows
    # once so the fresh dashboard contains the complete valid/test trajectory.
    if wandb_run is not None and existing_records and not had_persisted_wandb_id:
        for old in existing_records:
            payload = {
                "solution_step": int(old["solution_step"]),
                "comparison/wall_clock_seconds": float(old["wall_clock_seconds"]),
                "solutions/total_valid_mcc": float(old["total_valid_mcc"]),
                "solutions/total_mcc_valid": float(old["total_valid_mcc"]),
                "solutions/best_total_valid_mcc": float(old["best_total_valid_mcc"]),
            }
            for dataset_id in dataset_ids:
                current = old["current"][dataset_id]
                best = old["best"][dataset_id]
                payload.update({
                    f"solutions/metrics/{dataset_id}/valid_mcc": float(current["valid_mcc"]),
                    f"solutions/metrics/{dataset_id}/test_mcc": float(current["test_mcc"]),
                    f"solutions/best/{dataset_id}/valid_mcc": float(best["valid_mcc"]),
                    f"solutions/best/{dataset_id}/test_mcc": float(best["test_mcc"]),
                    f"comparison/fit_seconds/{dataset_id}": float(current["fit_seconds"]),
                })
                _log_hparams(payload, f"solutions/config/{dataset_id}", current["config"])
            wandb_run.log(payload)
        print(
            f"[optuna-comparison] replayed {len(existing_records)} imported "
            "transductive solution rows into the new W&B run",
            flush=True,
        )
    ...
    for solution_step in range(args.n_trials):
        ...
        prior_best = max(
            (float(old["best_total_valid_mcc"]) for old in existing_records),
            default=-np.inf,
        )
        should_validate = (
            args.validation_eval_policy == "every-trial"
            or (
                args.validation_eval_policy == "on-source-improvement"
                and float(record["best_total_valid_mcc"]) > prior_best
            )
        )
        existing_records.append(record)
        if should_validate and validation_ids and solution_step not in completed_validation_steps:
            print(
                f"[optuna-comparison] solution {solution_step}: training validation "
                f"datasets (policy={args.validation_eval_policy})",
                flush=True,
            )
            ...
            for validation_index, validation_id in enumerate(validation_ids):
                ...
                try:
                    ...
                    print(
                        f"[optuna-comparison] validation failed {validation_id}: "
                ...
                _log_hparams(
                    validation_payload,
                    f"validation/{validation_id}/hparams",
                    predicted_config,
                )
            total_validation_mcc = _aggregate_metric(
                validation_results, validation_ids, "valid_mcc"
            )
            ...
            validation_record = {
                "solution_step": solution_step,
                "evaluation_policy": args.validation_eval_policy,
            ...
            completed_validation_steps.add(solution_step)
    ...
    # Freeze one source-only meta-model after the full, predeclared source
    # budget. Target dataset descriptors are inference inputs only: no target
    # BERNN fit, score, or fixed test is opened by this search process.
    if validation_ids:
        validation_datasets = {
            dataset_id: hp_search.load_dataset(dataset_id)
            for dataset_id in validation_ids
        ...
        checkpoint_path = args.output_dir / "final_joint_meta_model.pt"
        torch.save(meta_checkpoint, checkpoint_path)
        recommendation = {
            "selection_protocol": "source_only_final_budget",
            "source_solution_steps": int(args.n_trials),
            "target_scores_observed": False,
            "target_configs": predicted_configs,
            "meta_diagnostics": meta_diagnostics,
            "checkpoint": checkpoint_path.name,
        }
        _atomic_json(args.output_dir / "final_recommendations.json", recommendation)
        if wandb_run is not None:
            payload = {"solution_step": int(args.n_trials - 1)}
            for dataset_id, config in predicted_configs.items():
                _log_hparams(payload, f"final_recommendation/{dataset_id}", config)
            wandb_run.log(payload)
